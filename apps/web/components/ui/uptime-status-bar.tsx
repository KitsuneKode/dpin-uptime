...
import {
  ...
  TooltipProvider,
  TooltipTrigger,
} from '@dpin-uptime/ui/components/tooltip'
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      for (let j = 1; j < dayTicks.length; j++) {
        ...
        if (
          currentTick &&
          previousTick &&
          currentTick.status === 'Bad' &&
          previousTick.status === 'Good'
        ) {
          incidents++
        }
      }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-sm">
        {showLabels && (
          <>
            <div className="text-muted-foreground">{days} days ago</div>
          ...
          {dayStatuses.map((day, index) => (
            <Tooltip key={index}>
              <TooltipTrigger asChild>
                <div
                  className={`h-8 flex-1 cursor-pointer rounded-sm transition-colors ${getBarColor(day.status)}`}
                  style={{ minWidth: '2px' }}
                />
              </TooltipTrigger>
              <TooltipContent>
