...
import type { DashboardMetrics, ApiResponse, ResponseTimeData, UptimeStats } from '@/lib/types';
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        if (!response?.data) {
          return { data: mockDashboardMetrics, success: true, message: 'mock-fallback' } satisfies ApiResponse<DashboardMetrics>;
        }
        return response;
      } catch {
        return { data: mockDashboardMetrics, success: true, message: 'mock-fallback' } satisfies ApiResponse<DashboardMetrics>;
...
export const useDashboardUptimeStats = (): UseQueryResult<ApiResponse<UptimeStats[]>> => {
  return useQuery({
