...
export const monitorKeys = {
  ...
  list: (filters: Record<string, unknown>) =>
    [...monitorKeys.lists(), { filters }] as const,
...
export const useMonitor = (
  id: string,
): UseQueryResult<ApiResponse<Monitor>> => {
  return useQuery({
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const transformedData: ResponseTimeData[] = res.data.map(
          (item: any) => ({
            timestamp:
        ...
        console.log(
          '[useResponseTimeData] Transformed data:',
          transformedData.length,
          'points',
        )
        return { data: transformedData, success: true } as ApiResponse<
          ResponseTimeData[]
        >
      } catch (error) {
...
export const useUptimeStats = (
  monitorId: string,
  period: TimePeriod,
): UseQueryResult<ApiResponse<UptimeStats>> => {
  return useQuery({
...
export const useCreateMonitor = (): UseMutationResult<
  { success: boolean; message: string } & Partial<Monitor>,
  Error,
  CreateMonitorData
> => {
...
export const useUpdateMonitor = (): UseMutationResult<
  ApiResponse<Monitor>,
  Error,
  UpdateMonitorData
> => {
  ...
  return useMutation({
    ...
    onSuccess: (response, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: monitorKeys.detail(variables.id),
      })
    },
  })
}
...
export const useDeleteMonitor = (): UseMutationResult<
...
export const usePauseMonitor = (): UseMutationResult<
  ApiResponse<Monitor>,
  Error,
  string
> => {
...
export const useResumeMonitor = (): UseMutationResult<
  ApiResponse<Monitor>,
  Error,
  string
> => {
